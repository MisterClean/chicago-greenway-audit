...
const expected=new Map((await readFile('data/normalized/scored/scores.ndjson','utf8')).trim().split('\n').map(line=>{const row=JSON.parse(line);return[row.sid,row]}));
const decoded=spawnSync('tippecanoe-decode',[`${base}/greenways.pmtiles`],{encoding:'utf8',maxBuffer:20*1024*1024});assert.equal(decoded.status,0,decoded.stderr);
