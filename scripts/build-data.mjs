...
const bundle=process.argv[2]?await reviewedBundle(process.argv[2],sources.find(s=>s.file==='greenways.json').sha256):null;
const inventory=bundle?.inventory??prepareInventory(rows,ward.features[0]);
...
const candidateRows=await json('data/review/matches.json');
const versionSeed=JSON.stringify(bundle)+JSON.stringify(networkSource)+JSON.stringify(candidateRows)+JSON.stringify(sources)+await readFile('data/overrides.json','utf8')+await readFile('scripts/inventory.mjs','utf8')+await readFile('scripts/build-data.mjs','utf8')+await readFile('solver/src/lib.rs','utf8')+await readFile('solver/src/main.rs','utf8')+await readFile('scripts/reviewed-bundle.mjs','utf8');
const buildId=`ward35-${sources[0].retrieved_at.slice(0,10)}-${sha(versionSeed).slice(0,10)}`;
...
await mkdir('data/normalized',{recursive:true}); await mkdir(`${out}/details`,{recursive:true});await mkdir(`${out}/runs`,{recursive:true});
...
await writeFile('data/normalized/states.ndjson',(bundle?.states??[]).map(s=>JSON.stringify(s)).join('\n'));
await writeFile('data/normalized/segments.ndjson',(bundle?.segments??inventory.map(r=>({sid:r.sid,gw_mm:r.gw_mm,states:[],directions_resolved:false,inaccessible:false}))).map(s=>JSON.stringify(s)).join('\n')+'\n');
const result=spawnSync('cargo',['run','--release','--locked','--manifest-path','solver/Cargo.toml','--','data/normalized/states.ndjson','data/normalized/segments.ndjson','data/normalized/scored'],{stdio:'inherit'});
if(result.status!==0) throw new Error('Solver failed');
const scores=new Map((await readFile('data/normalized/scored/scores.ndjson','utf8')).trim().split('\n').map(line=>{const row=JSON.parse(line);return [row.sid,row]}));
const witnesses=new Map((await readFile('data/normalized/scored/witnesses.ndjson','utf8')).trim().split('\n').filter(Boolean).map(line=>{const row=JSON.parse(line);return [row.state,row]}));
...
for(const row of inventory) {
 ...
 if(bundle){
  ...
  const candidate=candidateRows.find(r=>r.sid===row.sid)?.candidates[0];
...
await writeFile('data/normalized/greenways.geojson',JSON.stringify({type:'FeatureCollection',features}));
await writeFile('data/normalized/audit.ndjson',audit.map(r=>JSON.stringify(r)).join('\n')+'\n');
await writeFile(`${out}/catalog.json`,JSON.stringify(catalog));
await cp('data/normalized/scored/summary.bin',`${out}/summary.bin`);
await cp('data/sources/ward35.geojson',`${out}/ward35.geojson`);
const tile=spawnSync('tippecanoe',['--output',`${out}/greenways.pmtiles`,'--layer','greenways','--minimum-zoom','9','--maximum-zoom','16','--no-feature-limit','--no-tile-size-limit','--no-simplification-of-shared-nodes','data/normalized/greenways.geojson'],{encoding:'utf8',env:{...process.env,TIPPECANOE_MAX_THREADS:'1'}});
